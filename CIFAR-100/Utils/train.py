...
class CIFAR100InstanceSample(datasets.CIFAR100):
    """
    CIFAR-100 with instance index and contrastive sampling (for CRD).
    """
    def __init__(
        self,
        root,
        ...
        self.cls_positive = [np.asarray(c) for c in self.cls_positive]
        self.cls_negative = [np.asarray(c) for c in self.cls_negative]
...
def load_dataset_crd(bz=64, k=4096):
    ...
    trainset_weak = CIFAR100InstanceSample(
        root='./../../data',
        train=True,
    ...
    trainset_strong = CIFAR100InstanceSample(
        root='./../../data',
        train=True,
    ...
    test_loader = torch.utils.data.DataLoader(testset, batch_size=bz, shuffle=False, num_workers=2)
...
def main(model, model_name):
    ...
    torch.save(best_model_wts, f"aug_model_pth/best_model_weights_{model_name}.pth")
...
if __name__ == "__main__":
    if __package__ is None:
        ...
        from models import wrn, shufflenet_v1, vgg, mobilenetv2, shufflenet_v2
        from imagenet_models import mobilenetv1
    else:
        from ..models import wrn, shufflenet_v1, vgg, mobilenetv2, shufflenet_v2
        from ..imagenet_models import mobilenetv1
    
    model, model_name = mobilenetv1.mobilenetv1()
