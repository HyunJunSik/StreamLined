...
def main(model, model_name):
    ...
    train_loader, test_loader, num_data = get_imagenet_dataloaders_strong(1024, 128, 32)
    criterion = LabelSmoothingCrossEntropy()
    optimizer = optim.AdamW(model.parameters(), lr=5e-4, betas=(0.9, 0.999), eps=1e-8, weight_decay=0.05)
    model.to(device)
    ...
    scheduler = warmup_cosine_annealing_scheduler(optimizer, warmup_epochs, epoch_length)
...
if __name__ == "__main__":
    if __package__ is None:
        ...
        from Foundation import ViT
    else:
        from ..Foundation import ViT
    
    model, model_name = ViT.ViT_T_patch16_224()
